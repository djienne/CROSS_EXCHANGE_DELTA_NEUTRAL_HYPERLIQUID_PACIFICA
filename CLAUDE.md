...
- Converts to APR: `rate × 24 × 365 × 100` — **correct for both venues.** Hyperliquid pays
  hourly (verified: 12,061 of 12,062 recorded gaps are exactly 1.0h) and Pacifica's
  `/api/v1/info` `funding_rate` is documented as "paid in the past funding epoch (hour)".
  Note `test/test_pacifica_funding.py:68` annualizes Pacifica with ×1095 (an 8h assumption)
  — **that test is wrong, not the production path.**
- Goes LONG on exchange with lower funding rate, SHORT on higher
- ⚠️ **Net APR = absolute difference between the two rates — this is GROSS, not net of cost.**
  There is no fee term anywhere in the entry filter (`hyperliquid_pacifica_hedge.py:326`).
  Verified round trip is 0.170% (HL taker 0.045% + Pacifica taker 0.040%, both legs, entry
  and exit). Break-even is `0.170% × (8760 / hold_hours)`, so at the default
  `hold_duration_hours: 8` the bot must earn **186% APR** simply to break even, against a
  `min_net_apr_threshold` of 5.0 — a 37x shortfall.
  This is not theoretical: `bot_state_hl_pacifica.json` records 11 cycles, all counted as
  successful, totalling **−$4.46**, with individual cycle PnLs of −0.45, −0.68 and −1.49.
  Either lengthen `hold_duration_hours` (72h drops break-even to 20.7%) or add a fee-aware
  gate. Raising the threshold alone cannot work — no pair sustains 372% APR.
