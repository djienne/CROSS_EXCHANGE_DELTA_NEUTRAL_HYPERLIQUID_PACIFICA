...
class PacificaClient:
    ...
    def __init__(
        ...
        api_private: str,
        slippage_bps: int = 50,
        # DEFAULT FLIPPED TO False. When True, a failed API call returns FABRICATED
        # data rather than raising: mark price falls back to 60000 (BTC) / 3000
        # (ETH) / 0.0 (everything else), equity to 10000.0, and position to a
        # made-up 0.1 BTC long.
        #
        # Every one of those is a live-money hazard:
        #   - a 0.0 mark price makes unrealized_pnl = -notional, tripping the
        #     stop-loss and force-closing a perfectly healthy hedge
        #   - a stale 60000 while BTC trades at 90000 oversizes the trade ~1.5x, so
        #     the second leg is rejected for margin and the first is left naked
        #   - a phantom 10000 equity passes the balance guard and opens against an
        #     account that is actually empty
        #
        # Fabricated market data must never reach a sizing or risk decision. Callers
        # that genuinely want best-effort display values can still opt in explicitly.
        ...
        # Stamp the load time so get_funding_rate's TTL starts now rather than
        # forcing a redundant refresh on the very first call.
        self._funding_loaded_at: Optional[float] = time.time()
    ...
    # Funding rates go stale fast; market specs (tick/lot/min-notional) do not.
    # Only the rate is refreshed on this TTL.
    FUNDING_CACHE_TTL_S = 300.0

    def get_funding_rate(self, symbol: str, max_age_s: Optional[float] = None) -> float:
        """Get the current funding rate for `symbol`.

        Refreshes from the API when the cached value is older than the TTL.

        This used to read `self._market_info` directly, which is populated ONCE by
        `_load_market_info()` in __init__ and never again. A container running for
        days therefore re-traded a funding rate frozen at start-up: the rate could
        flip sign and the bot would keep opening against it, paying funding on both
        legs while its own display still showed the original attractive spread.
        """
        ttl = self.FUNDING_CACHE_TTL_S if max_age_s is None else max_age_s
        last = getattr(self, "_funding_loaded_at", None)
        if last is None or (time.time() - last) > ttl:
            try:
                self._load_market_info()
                self._funding_loaded_at = time.time()
            except Exception as e:                       # noqa: BLE001
                # Keep serving the stale value rather than fabricating 0.0, but say
                # so loudly -- a silent 0.0 reads as "no funding", which is a real
                # rate and would be acted upon.
                age = "unknown" if last is None else f"{time.time() - last:.0f}s"
                print(f"Warning: funding refresh failed for {symbol} ({e}). "
                      f"Serving cached rate (age {age}).")
                if last is None:
                    raise RuntimeError(
                        f"No funding rate available for {symbol} and refresh failed"
                    ) from e
