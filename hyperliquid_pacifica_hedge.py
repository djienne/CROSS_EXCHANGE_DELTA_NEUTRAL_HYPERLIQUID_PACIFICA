...
file_handler = RotatingFileHandler(
    'logs/hyperliquid_pacifica_hedge.log',
    mode='a', maxBytes=10 * 1024 * 1024, backupCount=3, encoding='utf-8',
)
...
class RotationBot:
    ...
    async def open_position(self, opportunity: Dict, notional: float):
        ...
        try:
            ...
            # Pass the symbol explicitly: current_position is not written until BOTH
            # legs succeed, so without it this call finds nothing in state and closes
            # nothing -- leaving whichever leg did fill naked and unrecorded.
            await self.close_position(is_emergency=True, symbol_override=symbol)
            self.state_mgr.set_state(BotState.ERROR)
    ...
    async def close_position(self, is_emergency: bool = False, max_retries: int = 3,
        ...
        `symbol_override` exists for the emergency path. current_position is only
        written AFTER both legs open successfully, so when open_position fails
        midway and calls close_position(is_emergency=True), there is nothing in
        state yet -- and this function used to log "Close called but no position in
        state" and return, having closed NOTHING while one leg was live.

        That is why an emergency unwind could print success over an open position.
        With an explicit symbol we query both venues directly and close whatever is
        actually there, regardless of what state believes.
        """
        if not is_emergency: self.state_mgr.set_state(BotState.CLOSING)
        pos = self.state_mgr.state["current_position"]

        if not pos:
            if is_emergency and symbol_override:
                logger.warning(
                    f"{Colors.YELLOW}Emergency close with no position in state - "
                    f"querying both venues directly for {symbol_override}.{Colors.RESET}"
                )
                symbol = symbol_override
                pos = {"symbol": symbol}          # minimal shim; only 'symbol' is used below
            else:
